...
def _round_numeric_features(
    df: pd.DataFrame, *, keys: tuple[str, str], decimals: int
) -> pd.DataFrame:
    idc, yc = keys
...
def _profile_gdf(gdf: gpd.GeoDataFrame, *, crs: str) -> None:
    ...
    try:
        ...
        logger.info(
            "GDF profile: rows={}, empty/NaN={}, types={}, CRS={}", n, n_empty, geom_types, crs
        )
...
def _profile_features(df: pd.DataFrame, *, keys: tuple[str, str]) -> None:
    ...
    logger.info(
        "Features matrix: rows={}, features={}, duplicate_keys={} ({}%)",
        n,
        len(cols),
        int(dup),
        f"{(dup / max(n, 1)) * 100:.2f}",
    )
    if len(nan_ratio):
...
def _connect_duckdb(
    db_path: Path, *, threads: int | None, mem: str | None
...
def _read_parcels_as_gdf(
    ...
    *,
    ...
    """).fetch_df()
    logger.info(
        "Fetched {} rows from DuckDB in {}", len(df), _fmt_seconds(time.perf_counter() - t0)
    )
...
def _available_feature_makers(makers_cfg: list[str] | None) -> list[Any]:
    all_names = [
        "SizeScaleFeatures",
...
def _run_maker(inst, gdf: gpd.GeoDataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        ncols = out.shape[1]
        nan_share = (
            (out.isna().sum().sum() / (out.shape[0] * max(out.shape[1], 1))) if out.size else 0.0
        )
...
def _compute_features(
    ...
    *,
    ...
    out = pd.concat(
        [gdf[[id_col, year_col]].reset_index(drop=True), feat.reset_index(drop=True)], axis=1
    )
...
def _write_features_to_duckdb(
    ...
    *,
    ...
    logger.success(
        'Table {}."{}" ready for joins USING ({}, {}), write_time={}',
        schema,
        dst_table,
        id_col,
        year_col,
...
def run_add_geometric_features(cfg: DictConfig) -> None:
    ...
    crs = str(
        _cfg_get(cfg, "_global_.crs_target", _cfg_get(cfg, "features.crs_target", "EPSG:2180"))
    )

    # Parametry kroku
    schema = _cfg_get(
        cfg, "features.add_geometric_features.schema", _cfg_get(cfg, "duckdb.schema", "egib")
    )
    ...
    mi_samples = int(
        _cfg_get(cfg, "features.add_geometric_features.moment_inertia.sample_points", 500)
    )
    ...
    try:
        ...
        if _cfg_get(cfg, "features.add_geometric_features.add_to_duckdb.enabled", True):
            _write_features_to_duckdb(
                con, df_feat, schema=schema, dst_table=dst_table, id_col=id_col, year_col=year_col
            )
        else:
